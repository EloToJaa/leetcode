...
// the number of keys exceeds the capacity from this operation, evict the least
// recently used key.
//
//
...
class LRUCache {
 private:
  ...
  size_t capacity;
  list<Entry> entries;
  unordered_map<int, list<Entry>::iterator> by_key;
 ...
 public:
  ...
  void put(int key, int value) {
    ...
    by_key.emplace(key, entries.begin());

    if (by_key.size() <= capacity) return;

    by_key.erase(entries.back().first);
    entries.pop_back();
  }
};
